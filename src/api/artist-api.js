...
import {
 ArtistTemplateSpec, ArtistDBSpec, ExampleArrays, IdSpec,
  ...
  deleteOne: {
    ...
    handler: async (request, h) => {
      try {
        ...
        switch (success) {
          case -1:
            return Boom.badRequest("Missing rights to delete this artist.");
          case 0:
            return Boom.badImplementation(`No artist with id ${request.params.id} => could not be deleted`);
          default:
    ...
    notes:
      "Deletes a specific artist when the command is executed by an Admin \
  ...
  deleteAll: {
    ...
    handler: async (request, h) => {
      try {
        ...
        switch (success) {
          case -1:
            return Boom.badRequest("Missing right to delete all artists.");
          default:
            return h.response(success).code(204);
        }
      } catch (err) {
